...
def sobel_perception(inputs: Array, use_laplace=False, key=None):
    ...
    if use_laplace:
        state_lap = jax.vmap(conv2d, in_axes=(0, None))(inputs, laplace)
...
def laplace_perception(inputs: Array, key=None):
    return  jax.vmap(conv2d, in_axes=(0, None))(inputs, laplace)
